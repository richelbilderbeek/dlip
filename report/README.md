...
1. We could use neural networks in phylogenetics to simulate animals [how?] and make a phylogeny out of that [how?], so you could see how animals speciate. This would create a very interersting [typo] phylogeny because you can then see where the species go extinct [how do species go extinct then?]. You can use this information to make predictions about real phylogenies [how?].

2. We could also use a neural network to find the parameters for the maximum likelihood function from the data in the tree [how?]. After you have found the parameters you could input the data of the tree and the parameters, and then it could guess the maximum likelihood formula [how? does this make sense? elaborate].

### How could we do that

1. You first have to make a realistic [define 'realistic'] simulation using a neural network of animals [so, each animal is one neuron? if not: how?], this has been done many times [references?]. You then need to find a way to get it to run quick enough to get a phylogeny without having to wait too long [redundant statement. Or can you give the big-O complexity?]. This might be hard because you would need a lot of computing power to run the simulations [redundant]. Therefor [typo] you will need very simple animals, or maybe use a model which doesn't simulate individual animals but does simulate a species [how? what are the characteristics of a species? how does the neural network power these? why use a neural network?] and how it develops [how?]. You can then compare these trees [how are these generated?] to real trees [where and how to find real trees?] and look at how the extinction rate influences the trees [uh, extinction causes less taxa in the present? elaborate or else this is redundant]. Then you can use this to improve concluded trees [how?].

2. You can use generated phylogenies [how? which models? which tools? what kinds of data?] with predefined parameters (as seen below) to feed to the neurons (the number of neurons is dependent on the number of taxons) [how to feed a phylogeny to a  neural network exactly?] and train it with them [how do you train?]. So the input would be the trees edges [how can a tree edge be an input for a neural network?] and the length [typo] of them. Then the neural network should be trained so, that it guesses the speciation and extionction parameters [great! how?]. Then, by having the input of the tree and the parameters, the neural network could guess the maximum likelihood formula [if it guesses the ML formula, how to train it on unknown ML formula's?].

For example, you could use a python script with tensorflow to make a neural network [what size? which number of input, hidden and output layers? what is the input? what is the output? what type of neurons? which transfer function? which speciation model?], then feed the data from the phylogeny (which you can generate in R or use method 1) to all the neurons [again, more details please]. Then train it in a way, that it then generates two outputs, your speciation rate, and your extinction rate.
